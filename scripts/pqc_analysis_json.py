...
def analyse_iv_data(path, options=None, config=None):
    ...
    timestamp_abs = np.array(
        list(map(rel_to_abs_timestamp, repeat(start_timestamp), timestamp))
    )
    rawdata.add_data(
        {
            "len": len(v),
...
def analyse_cv_data(path, options=None, config=None):
    ...
    if "Flute_1" in path:
        ...
        area = 1
        print(
            "WARNING: clould not determine flute number - area dependent values will be wrong!"
        )
    ...
    (
        ...
        abs(v), c, area=area, carrier="holes", **kwargs
    )  # cut_param= 0.008 for tracker, 0.03 for 120um

    annotate = (
        "V$_{{fd}}}}$: {} V\n\nT$_{{avg}}$: {} \u00B0C\nH$_{{avg}}$: {}".format(
            v_dep2, round(np.mean(temp), 2), round(np.mean(humidity), 2)
        )
        + r"$\%$"
    )
    ...
    if options.plot:
        ...
        fig.text(
            ...
            0.33,
            resstr,
            bbox=dict(facecolor="deepskyblue", alpha=0.75),
    ...
    if options.print:
        # print(f"{lbl}: CV: v_fd: {}")
        print(
            "%s: \tCV: v_fd: %.2e V\trho: %.2e Ohm\tconc: %.2e cm^-3"
            % (lbl, v_dep2, rho, conc * 1e-6)
        )
    meta = read_json_file(path).get("meta")
...
def analyse_mos_data(path, options=None, config=None):
    ...
    annotate = "V$_{{fb}}$: {} V (via intersection)\nV$_{{fb}}$: {} V (via inflection)\n\nt$_{{ox}}$: {} m\nn$_{{ox}}$: {} cm$^{{-2}}$".format(
        round(v_fb2, 2), round(v_fb1, 2), round(t_ox, 2), round(n_ox, 2)
    )
    ...
    if options.print:
        print(
            "%s: \tMOS: v_fb2: %.2e V\tc_acc: %.2e F\tt_ox: %.3e um\tn_ox: %.2e cm^-2"
            % (lbl, v_fb2, c_acc_m, t_ox, n_ox)
        )
...
def analyse_gcd_data(path, options=None, config=None):
    ...
    if options.plot:
        ...
        plot_curve(
            ...
            v,
            i_em * 1e12,
            options.plotTitle("GCD??"),
            "Gate Voltage / V",
            "Leakage Current / {}".format("pA"),
        )
        try:
    ...
    if options.print:
        print(
            "%s: \tGCD: i_surf: %.2e A\t i_bulk: %.2e A"
            % (lbl, gcd_result.i_surf, gcd_result.i_bulk)
        )
...
def analyse_fet_data(path, options=None, config=None):
    ...
    if options.plot:
        ...
        lns1a = ax1.plot(
            ...
            ms=3,
            color="tab:green",
            label="transfer characteristics - shifted",
        )
        lns1 = ax1.plot(
            v, i_em * 1e6, ls="", marker="s", ms=3, label="transfer characteristics"
        )
...
def analyse_van_der_pauw_data(path, options=None, min_correlation=0.99, config=None):
    ...
    r_sheet, a, b, x_fit, spl_dev, status, r_value = analyse_van_der_pauw(
        i, v, **kwargs
    )
    ...
    if options.print:
        ...
        print(
            "%s: \tvdp: r_sheet: %.2e Ohm/sq, raw: %.2e Ohm, correlation: %.2e  %s"
            % (lbl, r_sheet, a, r_value, lbl_vdp)
        )  # lbl_vdp
...
def analyse_linewidth_data(
    path, r_sheet=np.nan, options=None, min_correlation=0.9, config=None
):
    test = "linewidth"
    ...
    lbl = assign_label(path, test)
    lbl_vdp = assign_label(path, test, vdp=True)
    t_line, a, b, x_fit, spl_dev, r_value, status = analyse_linewidth(
        i,
        v,
        r_sheet=r_sheet,
        cut_param=-1.0,
...
def analyse_cbkr_data(
    path, r_sheet=np.nan, options=None, min_correlation=0.95, config=None
):
    test = "cbkr"
    ...
    r_contact, a, b, x_fit, spl_dev, r_value, status = analyse_cbkr(
        i, v, r_sheet, cut_param=0.01, debug=0, **kwargs
    )
...
def analyse_contact_data(path, options=None, min_correlation=0.95, config=None):
    ...
    lbl = assign_label(path, test)
    r_contact, a, b, x_fit, spl_dev, status, r_value = analyse_contact(
        i, v, cut_param=0.01, debug=0, **kwargs
    )
    ...
    if options.print:
        print(
            "%s: \tcontact: r_contact: %.2e Ohm, r_value: %.2f"
            % (lbl, r_contact, r_value)
        )
...
def analyse_breakdown_data(path, options=None, config=None):
    ...
    if options.plot:
        fig, ax = plt.subplots(1, 1)
        annotate = "V$_{{bd}}$: {} V \n\nT$_{{avg}}$ : {} \u00B0C \nH$_{{avg}}$: {} $\%$ ".format(
            v_bd, round(np.mean(temp), 2), round(np.mean(humidity), 2)
        )
        plot_curve(
            ax,
            v,
...
def get_vdp_value(pathlist, printResults=False, plotResults=False):
    ...
    for f in pathlist:
        # print(f)
        rs = analyse_van_der_pauw_data(f, min_correlation=0.7)
...
def parse_args():
    ...
    parser.add_argument(
        "-p", dest="plot", action="store_true", help="plot results in window"
    )
